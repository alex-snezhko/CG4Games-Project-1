...
/* input model data */
let gl; // the all powerful gl object. It's all here folks!
let allBuffers = { teapot: null, plane: null, person: null };
...
let eyePositionULoc;
let lightAmbientULoc;
let lightDiffuseULoc;
let lightSpecularULoc;
let lightPositionULoc;
...
let firstPassULoc; // where to put using texture boolean for fragment shader
let depthTexULoc; // where to put texture for fragment shader
let drawingOffsetULoc;
let drawingOffsetTexture;
let pencilTextureULoc;
let pencilTexture;
let randomValULoc;
let vPaperPosAttribLoc;
let paperTexULoc;
let paperTexture;
let paperRandomValXULoc;
let paperRandomValYULoc;
let mainProgram;
let paperProgram;
...
let LookAt = vec3.clone(defaultLookAt);
let Up = vec3.clone(defaultUp); // view up vector in world space
let eyeYAngle = 0;
let moveMode = "free-rotate";
const allMoveModes = ["free-rotate", "continous-rotate", "move"];
let selectedModel = "teapot";
const allModels = ["teapot", "plane", "person"];
let selectedShading = "pencil-sketch";
const allShadingModes = ["blinn-phong", "cel-shading", "pencil-sketch"];
...
function setupWebGL() {
    ...
    const webGLCanvas = document.getElementById("myWebGLCanvas"); // create a webgl canvas
...
function loadTextureNoMipmap(textureUrl) {
    // load a 1x1 gray image into texture for use when no texture, and until texture loads
    const texture = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D, texture); // activate model's texture
    // gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true); // invert vertical texcoord v, load gray 1x1
...
function createBuffers(vertices, normals, uvs, triangles, material) {
    ...
    const buffers = { vertsBuf, normsBuf, trisBuf, uvsBuf, numTris, material };
    return buffers;
}
async function loadObjs() {
    // AIRCO DH2 v2 by Joshua Johanson [CC-BY] via Poly Pizza
    const models = [["teapot", "http://localhost:8000/teapot.obj"], ["plane", "http://localhost:8000/plane.obj"]];
    for (const [model, url] of models) {
        const verts = [];
        const allNormals = [];
        const allUvs = [];
        const allTriVertUvNorms = [];
        ...
        for (let line of lines) {
            ...
            function getV3(strs) {
                const v3 = strs.map(parseFloat).slice(0, 3);
                return v3;
            }
            const first = words[0];
            ...
            if (first == "v") {
                ...
                allUvs.push(data.map(parseFloat).slice(0, 2));
            }
            else if (first == "f") {
                const vertUvNormIndexes = data.map(d => d.split("/").map(x => parseInt(x) - 1));
                const numTris = data.length - 2;
                for (let i = 0; i < numTris; i++) {
                    const tri = [vertUvNormIndexes[0], vertUvNormIndexes[i + 1], vertUvNormIndexes[i + 2]];
        ...
        for (const data of allTriVertUvNorms) {
            ...
            const vertIs = data.map(x => x[0]);
            tris.push(vertIs);
        }
        const material = { ambient: [0.5, 0.5, 0.5], diffuse: [0.3, 0.3, 0.3], specular: [0.2, 0.2, 0.2], n: 2, texture: null };
        allBuffers[model] = createBuffers(verts, norms, uvs, tris, material);
    }
    const allModelData = allModels.map(m => [m, document.getElementById(m)]);
    ...
    const allMoveModesData = allMoveModes.map(m => [m, document.getElementById(m)]);
    ...
    const allShadingData = allShadingModes.map(m => [m, document.getElementById(m)]);
...
function selectShadingMode(shadingMode) {
    selectedShading = shadingMode;
    setupShaders();
    // reset();
}
function createProgram(vShaderSrc, fShaderSrc) {
...
function renderScene(mMatrix, hpvmMatrix) {
...
function makeFBO() {
    ...
    gl.framebufferTexture2D(gl.FRAMEBUFFER, attachmentPoint, gl.TEXTURE_2D, depthTexture, level);
